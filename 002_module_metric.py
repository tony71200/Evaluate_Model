...
if __name__ == "__main__":
    ...
    Model_folder = "003_Model_20210809_RGB"
    confident_list = [0.01, 0.1, 0.5]
    iou_list = [0.1]
    ...
    for conf in confident_list:
        ...
        for iou in iou_list:
            result = eval.get_confusion(conf,iou,0.7,0.7, False)
            print(result)
            with open(os.path.join(eval.save_result, str(conf), "confusion_maxtrix_{}_iouThr_{}.txt".format(now.strftime("%Y%m%d"), iou)), 'w+') as writeLine:
