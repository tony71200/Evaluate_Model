...
class ObjectDetectMetric():
    def __init__(self, dir:str, path_image:str,folder_GroundTruth:str, 
        ...
        self.ground_truth = self._load_groundtruth(path_gt, is_nckuh)
    ...
    @staticmethod
    def _load_groundtruth(path_groundTruth, is_nckuh = False):
        if is_nckuh:
            path_dir = utils.get_dirname(path_groundTruth)
            list_gt = ['ggo_Test_B_w.csv', 'ggo_Test_B_wo.csv', 'ggo_Test_C_w.csv', 'ggo_Test_C_wo.csv']
            dict_gt = {}
    ...
    def filter_draw(self, conf_threshold, iou_thresh= 0.5, gt_color= 'red', pred_color = 'blue'):
        ...
        for path_image in self.list_path_image:
            ...
            if filename in self.prediction_NMS and len(self.prediction_NMS[filename]) != 0:
                pred_bb = np.array(self.prediction_NMS[filename])[:, 1:5]
                image = utils.drawBB(image, pred_bb, pred_color)
            ...
            if filename in self.TP:
                for index, (gt_bbox, pred_bbox) in enumerate(self.TP[filename]):
                    ...
                    bb = [x1, y1, x2, y2]
                    utils.crop_n_save(image, filename, bb, path_tp, index)
            ...
            if filename in self.FN:
                for index, gt_bbox in enumerate(self.FN[filename]):
                    utils.crop_n_save(image, filename, gt_bbox, path_fn, index)
            ...
            if filename in self.FP:
                for index, pred_bbox in enumerate(self.FP[filename]):
                    utils.crop_n_save(image, filename, pred_bbox, path_fp, index)
...
if __name__ == "__main__":
    ...
    path_image_folder = r'D:\02_BME\002_NCKUH\Db_20210816_RGB'
    directory = r'D:\02_BME\003_evaluation_Model\NCKUH'
    GT_folder = "000_GroundTruth"
    Model_folder = "006_Model_rgb_20210817"
    confident_list = [0.5]
    iou_list = [0.1]
    eval = ObjectDetectMetric(directory, path_image_folder, GT_folder, Model_folder,
