...
class ObjectDetectMetric():
    def __init__(self, dir:str, folder_GroundTruth:str, 
        ...
        path_pred = os.path.join(self.dir, self.folder_ModelName, jsonFile)
    ...
    def _processNMS(self, thresh_confident, thresh_iou = 0.5):
        for key, value in self.prediction.items():
            ...
            if len(delete_value) != 0:
                somelist = [i for j, i in enumerate(self.prediction[key]) if j not in np.unique(delete_value)]
                self.prediction[key] = somelist
    ...
    def _update(self, ground_truth, prediction):
        ...
        # print("match: {}".format(self.matched))
    ...
    def processAupdate(self, thresh_confident):
        self._processNMS(thresh_confident)
        for key in self.ground_truth.keys():
            gt = self.ground_truth[key]
            if key in self.prediction.keys():
                bb = self.prediction[key]
            else:
                bb = []
...
if __name__ == "__main__":
    ...
    Model_folder = "002_AllSubset_V2"
    jsonFile = "resultAllSubset_v2.json"
    eval = ObjectDetectMetric(directory, GT_folder, Model_folder,
                            ["nodule"], jsonFile= jsonFile)
    eval.processAupdate(0.5)
    eval.get_confusion(0.5,0.5,0.7,0.7, True)
